...
> The previous public GitHub layout is preserved in the `original-legacy`
...
The corpus spans motor imagery, ERP/SSVEP, cognitive and affective paradigms,
sleep, resting-state recordings, and neurological datasets collected with
heterogeneous montages and acquisition systems. Quality control standardized
the signals with 0.5--50 Hz band-pass filtering, power-line notch filtering,
resampling to 128 Hz, rejection of samples exceeding ±100 µV, and amplitude
scaling by `10^4`. Rejected samples define boundaries between continuous clean
segments, and pretraining sequences never cross those boundaries.
...
analysis/                validation, statistical tests, and README table generation
...
`summary.csv` files, with statistical markers read from the subject-level
paired analysis. Regenerate and verify the manuscript statistics before
...
The inferential analysis uses one accuracy per held-out subject, aligned by
exact subject ID. SingLEM (primary) is compared with each applicable non-SingLEM
baseline using a two-sided paired Wilcoxon signed-rank test. Holm correction is
applied separately within each dataset and evaluation setting, with significance
defined as Holm-adjusted `p < 0.05`. Accuracy is the inferential metric; macro-F1
and Cohen's kappa remain complementary descriptive metrics.

In the tables below, `*` denotes a baseline with significantly lower accuracy
than SingLEM (primary). SingLEM ablation variants are not part of the baseline
significance families. The complete [raw and Holm-adjusted p-value report](results/statistical_significance/p_value_report.md),
[comparison statistics and effect sizes](results/statistical_significance/paired_comparisons.csv),
[aligned subject-level accuracies](results/statistical_significance/aligned_subject_accuracies.csv),
and [analysis provenance](results/statistical_significance/analysis_config.json)
are published under [`results/statistical_significance/`](results/statistical_significance/).
