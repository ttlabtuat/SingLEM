...
class EEGEncoder(nn.Module):
    """
    EEG Encoder for EEG representation learning.
    ABLATED VERSION: This version removes the local-context
    embedding (SlidingWindow + Embedding Transformer) and uses a simple
    linear projection instead.
    """
    def __init__(self, config):
        super().__init__()
        self.config = config
        ...
        # --- ABLATION CHANGES START ---
        # The output of the CNN has dimension `token_len` (from the final conv layer).
        # We need to project this to `d_model` for the TransformerEncoder.
        self.feature_proj = nn.Linear(config.token_len, config.d_model) # Eventhough token_len and d_model have same dimensions, we apply this projection

        # We removed these embedding modules and helpers SlidingWindow+Sequencing
        # self.SlidingWindow = SlidingWindow(config)
        # self.Embedding = Embedding(config)
        # self.Sequencing = Sequencing(config)
        # --- ABLATION CHANGES END ---
    ...
    def forward(self, x):
        ...
        # --- ABLATION CHANGEST START ---
        features_seq = features.view(batch_size, current_seq_len, -1)   # self.Sequencing can be used
        projected_embs = self.feature_proj(features_seq)    # project from token-len -> d_model
        ...
        masked, mask_idx = self.Masking(projected_embs, current_seq_len)
